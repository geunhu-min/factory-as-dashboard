...
/**************************************************************
 * 스프레드시트를 열 때 메뉴 표시줄에 "이동" 메뉴를 추가합니다.
 * 예전 파일에는 이 메뉴가 onOpen으로 만들어져 있었는데, 새
 * 파일로 옮길 때 스크립트 프로젝트가 새로 시작되면서 같이 안
 * 넘어와서 메뉴 자체가 사라져 보였던 것입니다. 이 함수를 저장한
 * 뒤 스프레드시트를 새로고침하면 메뉴 표시줄에 다시 나타납니다.
 **************************************************************/
function onOpen() {
  SpreadsheetApp.getUi()
    .createMenu('이동')
    .addItem('마지막 행 다음 C열로 이동', 'moveToNextRowColumnC')
    .addToUi();
}


/**************************************************************
 * "이동" 메뉴용 매크로 — 예전 파일에서 매크로 기록으로 만들어져
